...
/**
 * ResourceLeakDetector<TimerWheel> leakDetector--->netty中内存泄漏监控对象管理者
 *      主要作用:
 *          在泄漏发生的时候，我们需要有个通知机制来知晓被GC的对象没有调用自己该有的release方法来释放池化资源
 *          需要有个记录机制来记录这些泄漏的对象使用的地方，方便溯源
 * ResourceLeak leak--->执行内存泄漏监控的对象
 * AtomicIntegerFieldUpdater<TimerWheel> WORKER_STATE_UPDATER--->netty中线程安全状态更新对象,主要使用CAP
 * Worker worker--->工作线程实际工作方法(包含时间轮的主要转动等流程)
 * Thread workerThread--->时间轮中的工作线程
 * int workerState--->时间轮实际工作状态 0 - init, 1 - started, 2 - shut down
 * long tickDuration--->每次转动轮子间隔的时间
 * HashedWheelBucket[] wheel--->时间槽
 * int mask--->最大的槽位(从0开始计算) = wheel.length -1 , 所有二进制位为1,时间轮的轮数为大于入参的2的n次方类似hashmap
 * CountDownLatch startTimeInitialized--->线程计数器(主要用于等待workerThread启动成功)
 * Queue<HashedWheelTimeout> timeouts--->时间轮中的所有任务
 * Queue<HashedWheelTimeout> cancelledTimeouts--->时间轮中取消的任务
 * long startTime--->时间轮的启动时间
 *
 *
 */
public class TimerWheel implements Timer {
    ...
    @Override
    public Timeout addTimeout(TimerTask task, long delay, TimeUnit unit, String argv) {
        if (task == null) {
    ...
    /**
     * 时间轮转动线程
     * long tick--->时针转动的总数,从定时任务开始之后开始累加, 也可以叫做滴答数
     *
     */
    private final class Worker implements Runnable {
        ...
        private void transferTimeoutsToBuckets() {
            ...
            for (int i = 0; i < 100000; i++) {
                ...
                //mask = 512 - 1, 每个byte位都是1, 进行&运算得到的结果 = calculated % wheel.length(512)
                //因为超过mask的byte位,都是512的倍数, 所以只要取后面的byte位的值就是正确的
     ...
     * TimerWheel timer--->所在时间轮
     * volatile int state--->任务状态,通过AtomicIntegerFieldUpdater线程安全更新对象中的Integer变量
     * HashedWheelTimeout next--->链表中的下一个对象
     * HashedWheelTimeout prev--->链表中的前一个对象
     * HashedWheelBucket bucket--->所在的时间轮槽位
     * TimerTask task--->定时任务对应的实际的处理流程
     * long deadline--->定时任务对于时间轮的到期时间
     *
     */
